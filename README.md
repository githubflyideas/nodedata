...
| 情况 | 鹈鹕 |
|---|---|
| 有机器不对 | 按 异常 → 失联 → 偏离 排好航线，一台一台飞过去，悬停拍翅，用探照灯照住那扇窗；左上角"巡检报告"显示这台的 USE 五行（读数、平时、从什么时候开始），身后横幅写"巡检中：pay-db-01 · 磁盘异常" |
| 失联 | 同上，头顶一个问号；报告里写拉不到的原因和该查什么 |
| 飞到时已经恢复 | 不停，直接去下一台 |
| 全部正常 | 沿楼顶一栋一栋滑翔巡视，拖着横幅"全部 N 台正常 · hh:mm" |
| host.list 写错了 | 横幅写哪一行错了 |
| 巡视台断开 | 落到最高那栋楼顶收起翅膀睡觉，城市变灰，横幅写最后一次更新时间 |

它身后有一条淡淡的虚线轨迹，是刚飞过的路。**它只去已经亮红、亮黄、变黑的窗，不推断任何原因。**
...
- 地址 `#` 后可以组合：`rack` / `group` / `product` / `dc` / `tags` 选分组，`sky` / `list` 选视图，`demo` 演示，用 `-` 连接。
