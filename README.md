...
页面、报告和巡视台上的 CPU / 内存 / 磁盘 / 网络 / 系统 五行，状态只有四种：正常、偏离、异常、无数据。
**大部分时间机器是正常的**，所以判定的第一目标是"正常的机器不亮黄"。

- **异常（红）**：L0 的绝对判定没过（只读挂载、分区满、OOM……），立刻亮。
- **偏离（黄）**，满足下面任意一条：
  1. **跟平时不一样，而且不一样得有问题**，三个条件同时满足：
     - 往**坏的方向**变。运行队列从 8 掉到 1、CPU 从 1.1% 掉到 0.5% 是变闲了，不算；
     - **显著**：|z| ≥ 3，而且变化量超过这个指标的最小变化量；
     - 读数本身**落在可能是瓶颈的区间**，并且**过去 60 秒一直在里面**。
  2. **绝对线**：读数本身就是结论，不管历史、不管 z 有没有就绪，持续 60 秒就报。
- **只标"比平时高/低"，不改变状态**：变化显著，但往好的方向，或者还没到可能成为瓶颈的程度。数据照样展示。

瓶颈区间和绝对线（Brendan Gregg 的 USE：打满、排队、出错才是问题；数字都在 `cmd/nodedata/concern.go`，每条写了来历）：

| 资源 | 瓶颈区间（配合 z） | 绝对线（不看历史） |
|---|---|---|
| CPU | 整机忙碌 ≥ 50%，单核 ≥ 60%，运行队列 > 核数，CPU 压力 ≥ 5%，被偷走 ≥ 5%，软中断最重的核 ≥ 30% | 单核 ≥ 95%，CPU 压力 ≥ 20%，运行队列 > 2 × 核数 |
| 内存 | 已用 ≥ 80%，换出/换入 ≥ 1 MB/s，内存压力 ≥ 5%，主缺页 ≥ 100/s，脏页/回写 ≥ 总内存 10% | 内存压力 ≥ 10%，换出 ≥ 10 MB/s |
| 磁盘 | IO 压力 ≥ 5%；延迟按盘的类型：机械盘 50ms、SSD 10ms、虚拟盘 20ms；util ≥ 60% 只对机械盘算；D 状态进程 ≥ 2 | IO 压力 ≥ 20% |
| 网络 | 丢包/错包 ≥ 1/s，TCP 重传 ≥ 10/s；吞吐不知道链路速率，只展示 | 丢包/错包持续 |

实测（`go test ./cmd/nodedata -run TestUse -v`，8 天历史回放）：机器"变闲"的场景，旧规则 61 次评估里亮黄 40 次，新规则 0 次；
真的打满照样报，不满 60 秒的尖峰不报；2 核机器上把一个核钉死，没有历史也在 1 分钟后报"单核打满（绝对线）"。
...
### 鹈鹕巡检 nodedata-pelican

同一套巡视台（同一份 host.list、同一套拉取和判断，代码在 `internal/fleet`），换一个"天空视图"做首页，默认端口 **8776**，
...
**整个机群是一座夜景城市**：一栋楼 = 一个机柜（顶部切到主机组 / 产品线 / 标签，楼就按那个分），一扇窗 = 一台主机，
按机房分城区。正常的窗亮冷绿，偏离亮黄，异常发红光、楼顶信标闪，失联的窗是黑的。100 台一屏看完。

**鹈鹕的航线就是巡检顺序，高度就是信号**：

| 情况 | 鹈鹕 |
|---|---|
| 全部正常 | 在高空（远高于楼顶）一栋一栋巡航，拖着横幅"全部 N 台正常 · hh:mm" |
| 异常 | 俯冲下来，贴着那扇窗悬停、快速拍翅，探照灯实心照住它 |
| 偏离 | 下到那栋楼顶上空，慢慢盘旋，探照灯较淡，一直盯着那扇窗 |
| 失联 | 贴窗悬停，头顶一个问号；报告里写拉不到的原因和该查什么 |
| 飞到时已经恢复 | 不停，直接去下一台 |
| host.list 写错了 | 横幅写哪一行错了 |
| 巡视台断开 | 落到最高那栋楼顶收起翅膀睡觉，城市变灰，横幅写最后一次更新时间 |

有问题时按 异常 → 失联 → 偏离 排航线；停下来时左上角"巡检报告"显示这台的 USE 五行（读数、平时、从什么时候开始）。
**它只去已经亮红、亮黄、变黑的窗，不推断任何原因。**
起点的口径跟主机页一样：巡视台亲眼看到变坏的写"hh:mm 起"，启动前就不正常的写"巡视台来之前就这样"。
天色跟 iPad 的本地时间走：白天 7–17 点蓝天、清晨和傍晚是霞光、其余是夜（地址加 `day` / `dusk` / `night` 可强制）。

- **点一栋楼**进这一组的列表页，**点一扇窗**进这台的主机页，左下角"← 回到天空"。
- 顶部"天空 / 列表"随时切换；列表视图就是 nodedata-fleet 的完整页面（轮播、方块、拉取状况）。
- **巡检日记**：巡视台亲眼看到的状态变化（正常→异常、失联、重新拉到了），底边显示最近几条，右上角看全部。只记什么时候变的，不猜为什么。
- **演示模式**：地址后加 `#demo`（如 `http://巡视台:8776/#rack-demo`），不连任何主机，本地生成 100 台随时间变化，
  顶部一直挂着"演示数据"。给还没部署的人看效果用。
- 地址 `#` 后可以组合：`rack` / `group` / `product` / `dc` / `tags` 选分组，`sky` / `list` 选视图，`day` / `dusk` / `night` 选天色，`demo` 演示，用 `-` 连接。
- 系统设置了"减少动态效果"时，鹈鹕不做飞行动画，直接出现在目标位置。
